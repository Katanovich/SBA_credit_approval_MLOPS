...
def group_by_first2_numeric(df, col):
    ...
    def mapper(x):
        if pd.isna(x): return 81
        if x in {11, 21, 22, 23, 42, 51, 52, 53, 54, 55, 56, 61, 62, 71, 72, 92}:
            return int(x)
        if 31 <= x <= 33: return 33
        if 44 <= x <= 45: return 45
        if 48 <= x <= 49: return 49
        return 81
    df[col] = first2.map(mapper)
    return df

def make_target(df, source_col="MIS_Status", target_col="target", positive_class="CHGOFF"):
    df = df.copy()
    df[source_col] = df[source_col].astype(str).str.upper().str.strip()
    df[target_col] = (df[source_col] == positive_class).astype(int)
    return df

def add_city_risk_group(df, city_col="City", target_col="target", min_loans=50, n_groups=3, labels=None):
    if labels is None: labels = ["LOW_RISK", "MEDIUM_RISK", "HIGH_RISK"]
    df = df.copy()
    if city_col not in df.columns or target_col not in df.columns:
        if "CityRiskGroup" not in df.columns: df["CityRiskGroup"] = "MEDIUM_RISK"
        return df
    try:
        city_stats = df.groupby(city_col).agg(TotalLoans=(target_col, "count"), DefaultRate=(target_col, "mean")).reset_index()
        city_stats = city_stats[city_stats["TotalLoans"] >= min_loans]
        if not city_stats.empty:
            city_stats["CityRiskGroup"] = pd.qcut(city_stats["DefaultRate"], q=n_groups, labels=labels)
...
def pre_pipeline_preparation(*, dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    data.drop(labels=config.model_config.unused_fields, axis=1, errors="ignore", inplace=True)
